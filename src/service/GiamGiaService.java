...
//    }
    public int getVoucher(String maVoucher) {
        int phanTramGiam = 0;
        double nguongApDung = 0; // Ngưỡng áp dụng của voucher

        // Kết nối cơ sở dữ liệu
        Connection con = DBConnect.getConnection();
        String sql = "SELECT [MaGiamGia], [TenChuongTrinh], [PhanTramGiamGia], [TrangThai], [TongTienApDung] "
                + "FROM [dbo].[GiamGia] WHERE MaGiamGia = ? AND TrangThai = 1"; // Giả sử cột 'NguongApDung' là ngưỡng voucher

        try {
            PreparedStatement stm = con.prepareStatement(sql);
            stm.setString(1, maVoucher); // Thiết lập giá trị mã voucher vào câu truy vấn
            ResultSet rs = stm.executeQuery();
            ...
            while (rs.next()) {
                ...
                // Lấy ngưỡng áp dụng của voucher
                nguongApDung = rs.getDouble("TongTienApDung"); // Giả sử bảng 'GiamGia' có cột 'NguongApDung'
            }
        ...
        // Lưu lại giá trị ngưỡng áp dụng để sử dụng sau (ví dụ: trả về cả phần trăm và ngưỡng)
        return phanTramGiam;
    }

    // Phương thức lấy ngưỡng áp dụng voucher
    public double getVoucherThreshold(String maVoucher) {
        double nguongApDung = 0; // Ngưỡng áp dụng của voucher

        // Kết nối cơ sở dữ liệu
        Connection con = DBConnect.getConnection();
        String sql = "SELECT [TongTienApDung] "
                + "FROM [dbo].[GiamGia] WHERE MaGiamGia = ? AND TrangThai = 1"; // Giả sử cột 'NguongApDung' là ngưỡng voucher
