...
public class BanHang extends javax.swing.JPanel implements Runnable, ThreadFactory {
    ...
    public void createPDF(String maHoaDon, String tenKH, String sdt, LocalDate ngayTao, double tongTien, double mucGiam, double tienKhachDua, javax.swing.JTable tblGH) throws Exception {
        // Đường dẫn lưu file
        String folderPath = "D:\\dự án 1 database\\Các File Lưu";
   ...
   String maGGToanCuc = null;
    ...
    private void btnthemvaogiohangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnthemvaogiohangActionPerformed
        ...
        double thanhTienMoi = gia * soLuong; // Thành tiền của lần thêm này
        ...
        for (HoaDonChiTiet gh : listGH) {
            if (maSach.equals(gh.getMaSach())) {
                ...
                double thanhTienCu = gh.getThanhTien();
                int slMoi = soLuong + slCu;
                double ttMoi = thanhTienCu + thanhTienMoi;

                // Kiểm tra nếu tổng tiền sau khi cập nhật vượt quá 500,000
                if (ttMoi > 500000) {
                    JOptionPane.showMessageDialog(null, "Tổng tiền cho sản phẩm này không được vượt quá 500,000. Vui lòng giảm số lượng sản phẩm!");
                    return;
                }

                hdService.capNhatSoLuongSanPham(maHD, maSach, slMoi, ttMoi);
        ...
        // Kiểm tra thành tiền không vượt quá 500,000 trước khi thêm mới
        if (thanhTienMoi > 500000) {
            JOptionPane.showMessageDialog(null, "Thành tiền không được vượt quá 500,000. Vui lòng giảm số lượng sản phẩm!");
            return;
        }

        // Thêm sản phẩm mới vào giỏ hàng
        LocalDate ngayTao = LocalDate.now();
        hdService.themSanPhamVaoGioHang(maHD, maSach, soLuong, gia, thanhTienMoi, ngayTao);
        double slSachCu = sachService.getSlSach(maSach);
        double slSachMoi = slSachCu - soLuong;
        sachService.updateSoLuong(maSach, slSachMoi);
        loadTableSP();
...
// Hàm cập nhật giỏ hàng trong cơ sở dữ liệu
    ...
    private void updateShoppingCart(String maHD, String maSach, int soLuong, double donGia, double thanhTien) {
...
//                tong += tt;
//            }
//            txtgiamgia.setText(String.valueOf((gg * tong) / 100));
//            double giamGia = Double.parseDouble(txtgiamgia.getText());
